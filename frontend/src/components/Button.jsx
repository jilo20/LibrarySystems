...
function Button({content, variant, isSubmit=false,route, stylee, onClick,value}){
    ...
    return (
        <>
            ...
            <button type={isSubmit ? "submit" : "button"} value={value} onClick={onClick}
            ...
            <button type={isSubmit ? "submit" : "button"} value={value} onClick={onClick}
                className={` group text-gray-100  border-gray-100 rounded-lg smooth-avg ${stylee}`}>
                {content}
            </button>}
        </>
    );
